        ...
        <section className="py-16 md:py-32">
            <div className="mx-auto max-w-6xl px-6">
                <div className="mx-auto max-w-2xl space-y-6 text-center">
                    ...
                    <p>Choose the plan that fits your needs. All plans include full access to our platform.</p>
                </div>
                    ...
                    <div className="mt-8 grid gap-6 md:mt-20 md:grid-cols-3">
                        {plans.items
                          ...
                          .map((plan: any, index: number) => {
                            const isPopular = plans.items.length === 2 ? index === 1 : index === Math.floor(plans.items.length / 2); // Mark middle/higher priced plan as popular
                            const price = plan.prices[0];
                            const isCurrentPlan = userSubscription?.polarPriceId === price.id && userSubscription?.status === 'active';
                            
                            return (
                                <Card key={plan.id} className={`relative ${isPopular ? 'border-primary' : ''} ${isCurrentPlan ? 'border-green-500 bg-green-50/50' : ''}`}>
                                    ...
                                    <CardHeader>
                                        ...
                                        <span className="my-3 block text-2xl font-semibold">
                                            ${(price.amount / 100).toFixed(0)} / {price.interval || 'mo'}
                                        </span>
                                        ...
                                        <Button
                                            ...
                                            disabled={loadingPriceId === price.id || (isCurrentPlan && userSubscription?.status === 'active')}
                                        >
                ...
                {userSubscription && !plans?.items.some((plan: any) => plan.prices[0].id === userSubscription.polarPriceId) && (
